...
//Clusters interface
type Albs interface {
	ListClusterALBs(clusterNameOrID string) ([]ALBConfig, error)
	GetALB(albID string) (ALBConfig, error)
	ConfigureALB(albID string, config ALBConfig) error
	RemoveALB(albID string) error
	DeployALBCert(config ALBSecretConfig) error
	UpdateALBCert(config ALBSecretConfig) error
	RemoveALBCertBySecretName(clusterID string, secretName string) error
	RemoveALBCertByCertCRN(clusterID string, certCRN string) error
	GetClusterALBCertBySecretName(clusterID string, secretName string) (ALBSecretConfig, error)
	GetClusterALBCertByCertCRN(clusterID string, certCRN string) (ALBSecretConfig, error)
	ListALBCerts(clusterID string) ([]ALBSecretConfig, error)
	GetALBTypes() ([]string, error)
}
...
func (r *alb) ListClusterALBs(clusterNameOrID string) ([]ALBConfig, error) {
...
func (r *alb) GetALB(albID string) (ALBConfig, error) {
...
func (r *alb) RemoveALB(albID string) error {
	_, err := r.client.Delete(fmt.Sprintf("/v1/alb/albs/%s", albID))
	return err
}
...
func (r *alb) RemoveALBCertBySecretName(clusterID string, secretName string) error {
	_, err := r.client.Delete(fmt.Sprintf("/v1/alb/clusters/%s/albsecrets?albSecretName=%s", clusterID, secretName))
	return err
}

// RemoveALBCertByCertCRN removes the alb-cert
func (r *alb) RemoveALBCertByCertCRN(clusterID string, certCRN string) error {
	_, err := r.client.Delete(fmt.Sprintf("/v1/alb/clusters/%s/albsecrets?certCrn=%s", clusterID, certCRN))
	return err
}

// GetClusterALBCertBySecretName returns details about specified alb cert for given secretName
func (r *alb) GetClusterALBCertBySecretName(clusterID string, secretName string) (ALBSecretConfig, error) {
...
func (r *alb) GetClusterALBCertByCertCRN(clusterID string, certCRN string) (ALBSecretConfig, error) {
	var successV ALBSecretConfig
	_, err := r.client.Get(fmt.Sprintf("/v1/alb/clusters/%s/albsecrets?certCrn=%s", clusterID, certCRN), &successV)
	return successV, err
}

// ListALBCerts for cluster...
func (r *alb) ListALBCerts(clusterID string) ([]ALBSecretConfig, error) {
...
func (r *alb) GetALBTypes() ([]string, error) {
	var successV []string
	_, err := r.client.Get("/v1/alb/albtypes", &successV)
	return successV, err
}
