...
func sendHTTPRequest(
	...
	if retries < 2 {
		return makeHTTPRequest(sess, path, requestType, bytes.NewBuffer(requestBody), options)
	}
...
func tryHTTPRequest(
	...
	resp, code, err := makeHTTPRequest(sess, path, requestType, bytes.NewBuffer(requestBody), options)
	if err != nil {
