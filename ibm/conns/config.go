...
func (session clientSession) ConfigurationGovernanceV1() (*configurationgovernancev1.ConfigurationGovernanceV1, error) {
	if session.configServiceApiClient != nil {
		return session.configServiceApiClient, session.configServiceApiClientErr
	}
	return session.configServiceApiClient.Clone(), nil
}
...
func (c *Config) ClientSession() (interface{}, error) {
	...
	configServiceApiClientOptions := &configurationgovernancev1.ConfigurationGovernanceV1Options{
		Authenticator: authenticator,
		URL:           EnvFallBack([]string{"IBMCLOUD_SCC_CONFIGURATION_GOVERNANCE_API_ENDPOINT"}, configServiceApiClientURL),
	}
	...
	if err == nil {
		...
		session.configServiceApiClientErr = fmt.Errorf("Error occurred while configuring Security and Compliance Center Config Governance API service: %q", err)
	}
