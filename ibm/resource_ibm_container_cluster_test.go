...
func TestAccIBMContainerClusterBasicDefaultPoolNameSet(t *testing.T) {
	clusterName := fmt.Sprintf("tf-cluster-%d", acctest.RandIntRange(10, 100))
	resource.Test(t, resource.TestCase{
		PreCheck:     func() { testAccPreCheck(t) },
		Providers:    testAccProviders,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckIBMContainerClusterBasicDefaultPoolNameSet(clusterName),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"ibm_container_cluster.testacc_cluster", "name", clusterName),
					resource.TestCheckResourceAttr(
			...
			{
				Config: testAccCheckIBMContainerClusterUpdateDefaultPoolNameSet(clusterName),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"ibm_container_cluster.testacc_cluster", "name", clusterName),
					resource.TestCheckResourceAttr(
...
func testAccCheckIBMContainerClusterBasicDefaultPoolNameSet(clusterName string) string {
	return fmt.Sprintf(`
...
func testAccCheckIBMContainerClusterUpdateDefaultPoolNameSet(clusterName string) string {
	return fmt.Sprintf(`
