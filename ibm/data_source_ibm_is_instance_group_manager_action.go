...
func dataSourceIBMISInstanceGroupManagerActionRead(d *schema.ResourceData, meta interface{}) error {
	log.Println("Inside dataSourceIBMISInstanceGroupManagerActionRead")
	...
	instanceGroupManagerID := d.Get("instance_group_manager_scheduled").(string)
	...
	for _, data := range allrecs {
		...
		if actionName == *instanceGroupManagerAction.Name {
			log.Println("value of  *instanceGroupManagerAction.Name ")
			log.Println(*instanceGroupManagerAction.Name)
			...
			if err = d.Set("instance_group_manager_action", *instanceGroupManagerAction.ID); err != nil {
	...
	return fmt.Errorf("instance group manager policy %s not found", actionName)
}
