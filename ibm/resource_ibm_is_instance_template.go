	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			isInstanceTemplatePlacementTarget: {
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						isInstanceTemplateResourceType: {
							Type:     schema.TypeString,
							Optional: true,
							Computed: true,
						},
...
func instanceTemplateCreate(d *schema.ResourceData, meta interface{}, profile, name, vpcID, zone, image string) error {
	...
	log.Println("Creating Instance Template")
	...
	log.Println("Creating Instance Template after volume attachment")
	...
	log.Println("Creating Instance Template after primary network interface")
	...
	log.Println("Creating Instance Template after network interface")
	...
	if placetargetintf, ok := d.GetOk(isInstanceTemplatePlacementTarget); ok {

		log.Println("Inside Create Instance placement")
		placeTarget := placetargetintf.([]interface{})[0].(map[string]interface{})
		if placeTarget[isInstanceTemplateResourceType] != nil {
			restype, _ := placeTarget[isInstanceTemplateResourceType]
			...
			// if restypestr == isInstanceTemplateDedicatedHost {
	...
	log.Println("Creating Instance Template after resource group")
	options := &vpcv1.CreateInstanceTemplateOptions{
		InstanceTemplatePrototype: instanceproto,
	}
	log.Println("Creating Instance Template calling api......")
...
func resourceIBMisInstanceTemplateCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		log.Printf("[DEBUG] Create Instance template err %s", err)
		return err
	}
...
func instanceTemplateGet(d *schema.ResourceData, meta interface{}, ID string) error {
	...
	if instance.PlacementTarget != nil {
		...
		if instance.PlacementTarget != nil {
			...
			placement := instanceProtoIntf.(*vpcv1.InstancePlacementPrototype)

			placeTarget["id"] = *placement.ID
		}
		placeTargetList = append(placeTargetList, placeTarget)
		d.Set(isInstanceTemplatePlacementTarget, placeTargetList)
	}
	return nil
}
