...
func TestAccIBMISSecurityGroupRuleDatasource_basic(t *testing.T) {
	...
	dataSourceName := "data.ibm_is_security_group_rule.sg1_rule"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			resource.TestStep{
				Config: testAccCheckIBMISSgRuleConfigDsRead(sgname),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet(dataSourceName, "rules.0.protocol"),
					resource.TestCheckResourceAttrSet(dataSourceName, "rules.0.direction"),
					resource.TestCheckResourceAttrSet(dataSourceName, "rules.0.ip_version"),
					resource.TestCheckResourceAttrSet(dataSourceName, "rules.0.rule_id"),
				),
			},
		},
	})
...
func testAccCheckIBMISSgRuleConfigDsRead(sgname string) string {
	return fmt.Sprintf(`
	data "ibm_is_security_group_rule" "sg1_rule" {
		name = "%s"
	}
	`, sgname)
