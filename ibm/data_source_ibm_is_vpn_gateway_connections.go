...
const (
	...
	isVPNGatewayConnectionsStatus = "status"
	isVPNGatewayID                = "vpn_gateway_id"
...
func dataSourceIBMISVPNGatewayConnections() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			isVPNGatewayConnectionsStatus: {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "Filters the collection to VPN gateway connections with the specified status ",
			},
			isVPNGatewayID: {
				Type:        schema.TypeString,
				Required:    true,
...
func dataSourceIBMVPNGatewayConnectionsRead(d *schema.ResourceData, meta interface{}) error {
	...
	if gwstatus, ok := d.GetOk(isVPNGatewayConnectionsStatus); ok {
		listvpnGWConnectionOptions.SetStatus(gwstatus.(string))
	}
