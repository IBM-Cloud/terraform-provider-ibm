...
func TestAccIBMContainerVpcClusterBasicDefaultPoolNameSet(t *testing.T) {
	name := fmt.Sprintf("tf-vpc-cluster-%d", acctest.RandIntRange(10, 100))
	var conf *v2.ClusterInfo
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckIBMContainerVpcClusterBasicDefaultPoolNameSet(name),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckIBMContainerVpcExists("ibm_container_vpc_cluster.cluster", conf),
					resource.TestCheckResourceAttr(
						"ibm_container_vpc_cluster.cluster", "name", name),
			...
			{
				Config: testAccCheckIBMContainerVpcClusterUpdateefaultPoolNameSet(name),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckIBMContainerVpcExists("ibm_container_vpc_cluster.cluster", conf),
					resource.TestCheckResourceAttr(
						"ibm_container_vpc_cluster.cluster", "name", name),
...
func testAccCheckIBMContainerVpcClusterBasicDefaultPoolNameSet(name string) string {
	return fmt.Sprintf(`
provider "ibm" {
	region ="eu-de"
}	
...
func testAccCheckIBMContainerVpcClusterUpdateefaultPoolNameSet(name string) string {
	return fmt.Sprintf(`
provider "ibm" {
	region ="eu-de"
}	
