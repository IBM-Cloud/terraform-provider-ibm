...
func TestAccIBMTektonPipelineAllArgs(t *testing.T) {
	var conf continuousdeliverypipelinev2.TektonPipeline
	integrationInstanceID := fmt.Sprintf("tf_integration_instance_id_%d", acctest.RandIntRange(10, 100))
	integrationInstanceIDUpdate := fmt.Sprintf("tf_integration_instance_id_%d", acctest.RandIntRange(10, 100))
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				Config: testAccCheckIBMTektonPipelineConfig(integrationInstanceID),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckIBMTektonPipelineExists("ibm_tekton_pipeline.tekton_pipeline", conf),
					resource.TestCheckResourceAttr("ibm_tekton_pipeline.tekton_pipeline", "integration_instance_id", integrationInstanceID),
				),
			},
			resource.TestStep{
				Config: testAccCheckIBMTektonPipelineConfig(integrationInstanceIDUpdate),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("ibm_tekton_pipeline.tekton_pipeline", "integration_instance_id", integrationInstanceIDUpdate),
				),
			},
			resource.TestStep{
...
func testAccCheckIBMTektonPipelineConfig(integrationInstanceID string) string {
	return fmt.Sprintf(`
