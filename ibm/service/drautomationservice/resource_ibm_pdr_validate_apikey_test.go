...
func testAccCheckIbmPdrValidateApikeyExists(n string, obj drautomationservicev1.ValidationKeyResponse) resource.TestCheckFunc {
	...
	return func(s *terraform.State) error {
		...
		validationKeyResponse, _, err := drAutomationServiceClient.ServiceInstanceGetKeyV1(serviceInstanceGetKeyV1Options)
		if err != nil {
			return err
		}
...
func testAccCheckIbmPdrValidateApikeyDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		// Try to find the key
		_, response, err := drAutomationServiceClient.ServiceInstanceGetKeyV1(serviceInstanceGetKeyV1Options)
