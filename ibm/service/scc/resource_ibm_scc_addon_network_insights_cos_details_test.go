...
func TestAccIBMSccAddonNetworkInsightsCosDetailsBasic(t *testing.T) {
	var conf addonmanagerv1.NetworkInsightsCosDetailsOutput
	regionID := "us"
...
func testAccCheckIBMSccAddonNetworkInsightsCosDetailsExists(n string, obj addonmanagerv1.NetworkInsightsCosDetailsOutput) resource.TestCheckFunc {
	...
	return func(s *terraform.State) error {
		...
		for _, cosDetail := range niCosDetailsV2Output.CosDetails {
			if cosDetail.ID == &parts[1] {
				cosDetails := addonmanagerv1.NetworkInsightsCosDetailsOutput{
