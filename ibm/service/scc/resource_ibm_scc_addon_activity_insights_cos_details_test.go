...
func TestAccIBMSccAddonActivityInsightsCosDetailsBasic(t *testing.T) {
	var conf addonmanagerv1.AiCosDetailsV2Output
	regionID := "us"
...
func testAccCheckIBMSccAddonActivityInsightsCosDetailsExists(n string, obj addonmanagerv1.AiCosDetailsV2Output) resource.TestCheckFunc {
	...
	return func(s *terraform.State) error {
		...
		for _, cosDetail := range aiCosDetailsV2Output.CosDetails {
			if cosDetail.ID == &parts[1] {
				cosDetails := addonmanagerv1.AiCosDetailsV2Output{
					RegionID: aiCosDetailsV2Output.RegionID,
					CosDetails: []addonmanagerv1.AiCosDetailsV2OutputCosDetailsItem{
						cosDetail,
					},
				}
				obj = cosDetails
