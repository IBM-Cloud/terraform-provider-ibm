	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			dlOfferingType: {
				...
				ValidateFunc: validate.InvokeDataSourceValidator("ibm_dl_offering_speeds", dlOfferingType),
			},
			dlSpeeds: {
...
func dataSourceIBMDLOfferingSpeedsRead(d *schema.ResourceData, meta interface{}) error {
	...
	for _, instance := range listSpeeds.Speeds {
		...
		if instance.Capabilities != nil {
			speed[dlMeteringCapabilities] = flex.FlattenStringList(instance.Capabilities)
		}
...
func DataSourceIBMDLOfferingSpeedsValidator() *validate.ResourceValidator {
	...
	ibmDLOfferingSpeedsDatasourceValidator := validate.ResourceValidator{ResourceName: "ibm_dl_offering_speeds", Schema: validateSchema}
