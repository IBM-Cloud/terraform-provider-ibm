		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet("ibm_is_backup_policy.is_backup_policy", "last_job_completed_at"),
...
func testAccCheckIBMIsBackupPolicyConfigBasic(backupPolicyName string, vpcname, subnetname, sshname, publicKey, volName, name string) string {
	...
	resource "ibm_is_vpc" "testacc_vpc" {
	  ...
	  resource "ibm_is_backup_policy" "is_backup_policy" {
		...
		name            = "%s"
	}`, vpcname, subnetname, acc.ISZoneName, acc.ISCIDR, sshname, publicKey, volName, acc.ISZoneName, name, acc.IsImage, acc.InstanceProfileName, acc.ISZoneName, backupPolicyName)
}
