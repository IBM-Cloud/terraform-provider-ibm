...
func TestAccIBMISInstanceTemplate_bootFirmwareSelectionMode(t *testing.T) {
	randInt := acctest.RandIntRange(10, 100)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckIBMISInstanceTemplateBootFirmwareConfig(vpcName, subnetName, sshKeyName, publicKey, templateName, bfsm),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"ibm_is_instance_template.instancetemplate1", "name", templateName),
					resource.TestCheckResourceAttr(
						"ibm_is_instance_template.instancetemplate1", "boot_firmware_selection_mode", bfsm),
				),
			},
		},
	})
...
func testAccCheckIBMISInstanceTemplateBootFirmwareConfig(vpcName, subnetName, sshKeyName, publicKey, templateName, bfsm string) string {
	return fmt.Sprintf(`
	resource "ibm_is_vpc" "vpc2" {
	  name = "%s"
	}
	...
	`, vpcName, subnetName, acc.ISZoneName, acc.ISCIDR, sshKeyName, publicKey, templateName, acc.IsImage, acc.InstanceProfileName, bfsm, acc.ISZoneName)
}
