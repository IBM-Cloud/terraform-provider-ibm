...
func TestAccIBMISInstanceTemplate_keys_update(t *testing.T) {
	randInt := acctest.RandIntRange(10, 100)
	...
	templateName := fmt.Sprintf("tf-testtemplate%d", randInt)
	sshKeyName := fmt.Sprintf("tf-testsshkey%d", randInt)
	sshKeyName2 := fmt.Sprintf("tf-testsshkey%d", randInt)
	resource.Test(t, resource.TestCase{
		PreCheck:     func() { acc.TestAccPreCheck(t) },
		Providers:    acc.TestAccProviders,
		CheckDestroy: testAccCheckIBMISInstanceTemplateDestroy,
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"ibm_is_instance_template.instancetemplate1", "name", templateName),
					resource.TestCheckResourceAttr(
						"ibm_is_instance_template.instancetemplate1", "keys.#", "1"),
					resource.TestCheckResourceAttrSet(
						"ibm_is_instance_template.instancetemplate1", "profile"),
				),
			},
			{
				Config: testAccCheckIBMISInstanceTemplateConfigKeyUpdate(vpcName, subnetName, sshKeyName, publicKey, sshKeyName2, sshKey, publicKey2, templateName),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"ibm_is_instance_template.instancetemplate1", "name", templateName),
					resource.TestCheckResourceAttr(
						"ibm_is_instance_template.instancetemplate1", "keys.#", "2"),
					resource.TestCheckResourceAttrSet(
						"ibm_is_instance_template.instancetemplate1", "profile"),
				),
			},
...
func testAccCheckIBMISInstanceTemplateConfigKeyUpdate(vpcName, subnetName, sshKeyName, publicKey, sshKeyName2, sshKey, publicKey2, templateName string) string {
	return fmt.Sprintf(`	
	resource "ibm_is_vpc" "vpc2" {
	  name = "%s"
	}
	...
	resource "ibm_is_ssh_key" "sshkey2" {
	  name       	= "%s"
	  public_key 	= "%s"
	  type 			= "%s"
	}
	...
	resource "ibm_is_instance_template" "instancetemplate1" {
	   ...
	   vpc       = ibm_is_vpc.vpc2.id
	   zone      = "us-south-2"
	   keys      = [ibm_is_ssh_key.sshkey.id, ibm_is_ssh_key.sshkey2.id]
	 }
		
	
	`, vpcName, subnetName, sshKeyName, publicKey, sshKeyName2, publicKey2, sshKey, templateName)
