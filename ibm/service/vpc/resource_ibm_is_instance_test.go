...
func TestAccIBMISInstance_licensing_basic(t *testing.T) {
	var instance string
	vpcname := fmt.Sprintf("tf-vpc-%d", acctest.RandIntRange(10, 100))
	name := fmt.Sprintf("tf-instnace-%d", acctest.RandIntRange(10, 100))
	subnetname := fmt.Sprintf("tf-subnet-%d", acctest.RandIntRange(10, 100))
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet(
						"ibm_is_instance.testacc_instance", "vcpu.0.manufacturer"),
					resource.TestCheckResourceAttrSet(
						"ibm_is_instance.testacc_instance", "software_attachments.#"),
				),
			},
			{
				Config: testAccCheckIBMISInstanceConfig(vpcname, subnetname, sshname, publicKey, name, userData2),
...
func TestAccIBMISInstanceAvailabilityPolicy_basic(t *testing.T) {
	var instance string
	vpcname := fmt.Sprintf("tf-vpc-%d", acctest.RandIntRange(10, 100))
	name := fmt.Sprintf("tf-instnace-%d", acctest.RandIntRange(10, 100))
	subnetname := fmt.Sprintf("tf-subnet-%d", acctest.RandIntRange(10, 100))
...
`)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckIBMISInstanceConfigWithAvailabilityPolicy_Default(vpcname, subnetname, sshname, publicKey, name),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckIBMISInstanceExists("ibm_is_instance.testacc_instance", instance),
					resource.TestCheckResourceAttr(
						"ibm_is_instance.testacc_instance", "name", name),
					resource.TestCheckResourceAttr(
						"ibm_is_instance.testacc_instance", "zone", acc.ISZoneName),
					resource.TestCheckResourceAttr("ibm_is_instance.testacc_instance", "availability.0.class", "spot"),
					resource.TestCheckResourceAttr("ibm_is_instance.testacc_instance", "availability_policy.0.preemption", "stop"),
					resource.TestCheckResourceAttr("ibm_is_instance.testacc_instance", "availability_policy.0.host_failure", "restart"),
				),
			},
			{
				Config: testAccCheckIBMISInstanceConfigWithAvailabilityPolicy_Updated(vpcname, subnetname, sshname, publicKey, name),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckIBMISInstanceExists("ibm_is_instance.testacc_instance", instance),
					resource.TestCheckResourceAttr(
						"ibm_is_instance.testacc_instance", "name", name),
					resource.TestCheckResourceAttr(
						"ibm_is_instance.testacc_instance", "zone", acc.ISZoneName),
					resource.TestCheckResourceAttr("ibm_is_instance.testacc_instance", "availability.0.class", "spot"),
					resource.TestCheckResourceAttr("ibm_is_instance.testacc_instance", "availability_policy.0.host_failure", "stop"),
					resource.TestCheckResourceAttr("ibm_is_instance.testacc_instance", "availability_policy.0.preemption", "delete"),
				),
			},
		},
	})
...
// Test for downsizing profile with bandwidth adjustment
func TestAccIBMISInstance_ProfileDownsizeWithBandwidth(t *testing.T) {
	var instance string
	vpcname := fmt.Sprintf("tf-vpc-%d", acctest.RandIntRange(10, 100))
	name := fmt.Sprintf("tf-instance-%d", acctest.RandIntRange(10, 100))
	subnetname := fmt.Sprintf("tf-subnet-%d", acctest.RandIntRange(10, 100))
...
`)
	sshname := fmt.Sprintf("tf-ssh-%d", acctest.RandIntRange(10, 100))
	initialProfile := "cx2-48x96" // Larger profile with high bandwidth capacity
	downsizedProfile := "cx2-4x8" // Smaller profile with lower bandwidth capacity
	initialBandwidth := 20000     // High bandwidth valid for larger profile
	downsizedBandwidth := 2000    // Reduced bandwidth to fit smaller profile
	prefix := fmt.Sprintf("tf-prefix-%d", acctest.RandIntRange(10, 100))
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccCheckIBMISInstanceConfigWithProfileAndBandwidth(
					vpcname, subnetname, sshname, publicKey, name, prefix, downsizedProfile, downsizedBandwidth),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckIBMISInstanceExists("ibm_is_instance.testacc_instance", instance),
					resource.TestCheckResourceAttr(
						"ibm_is_instance.testacc_instance", "name", name),
					resource.TestCheckResourceAttr(
						"ibm_is_instance.testacc_instance", "profile", downsizedProfile),
					resource.TestCheckResourceAttr(
						"ibm_is_instance.testacc_instance", "total_volume_bandwidth", fmt.Sprintf("%d", downsizedBandwidth)),
					resource.TestCheckResourceAttr(
						"ibm_is_instance.testacc_instance", "zone", acc.ISZoneName),
				),
			},
...
func testAccCheckIBMISInstanceConfigWithAvailabilityPolicy_Default(vpcname, subnetname, sshname, publicKey, name string) string {
	return fmt.Sprintf(`
	resource "ibm_is_vpc" "testacc_vpc" {
		name = "%s"
	  }
...
func testAccCheckIBMISInstanceConfigWithAvailabilityPolicy_Updated(vpcname, subnetname, sshname, publicKey, name string) string {
	return fmt.Sprintf(`
	resource "ibm_is_vpc" "testacc_vpc" {
		name = "%s"
	  }
