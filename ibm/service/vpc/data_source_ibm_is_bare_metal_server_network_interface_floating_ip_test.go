...
func TestAccIBMISBareMetalServerNetworkInterfaceFloatingIPDataSource_basic(t *testing.T) {
...
`)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				Config: testAccCheckIBMISBareMetalServerNetworkInterfaceFloatingIPDataSourceConfig(vpcname, subnetname, sshname, publicKey, name),
...
func testAccCheckIBMISBareMetalServerNetworkInterfaceFloatingIPDataSourceConfig(vpcname, subnetname, sshname, publicKey, name string) string {
	// status filter defaults to empty

	return testAccCheckIBMISBareMetalServerConfig(vpcname, subnetname, sshname, publicKey, name) +
		fmt.Sprintf(`
      data "ibm_is_bare_metal_server_network_interface_floating_ip" "test1" {
