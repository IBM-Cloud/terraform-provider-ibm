...
// TestAccIBMISSnapshot_allowedUse_bootFirmwareSelectionMode verifies that
// allowed_use.instance accepts boot_firmware_selection_mode as a CEL variable,
// both on create and on update to a different expression.
func TestAccIBMISSnapshot_allowedUse_bootFirmwareSelectionMode(t *testing.T) {
	vpcname := fmt.Sprintf("tf-vpc-%d", acctest.RandIntRange(10, 100))
	name := fmt.Sprintf("tf-instnace-%d", acctest.RandIntRange(10, 100))
	subnetname := fmt.Sprintf("tf-subnet-%d", acctest.RandIntRange(10, 100))
	publicKey := strings.TrimSpace(`
...
`)
	sshname := fmt.Sprintf("tf-ssh-%d", acctest.RandIntRange(10, 100))
	sname := fmt.Sprintf("tfsnapshotuat-%d", acctest.RandIntRange(10, 100))
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckIBMISSnapshotBootFirmwareConfig(vpcname, subnetname, sshname, publicKey, name, sname, instanceCreate),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet(
						"ibm_is_snapshot.testacc_snapshot", "allowed_use.#"),
					resource.TestCheckResourceAttr(
						"ibm_is_snapshot.testacc_snapshot", "allowed_use.0.instance", instanceCreate),
				),
			},
			{
				Config: testAccCheckIBMISSnapshotBootFirmwareConfig(vpcname, subnetname, sshname, publicKey, name, sname, instanceUpdate),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"ibm_is_snapshot.testacc_snapshot", "allowed_use.0.instance", instanceUpdate),
				),
			},
		},
	})
...
func testAccCheckIBMISSnapshotBootFirmwareConfig(vpcname, subnetname, sshname, publicKey, name, sname, instanceExpr string) string {
	return fmt.Sprintf(`
resource "ibm_is_vpc" "testacc_vpc" {
  name = "%s"
}
