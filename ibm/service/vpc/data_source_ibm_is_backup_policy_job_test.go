...
`)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				Config: testAccCheckIBMIsBackupPolicyJobDataSourceConfigBasic(bakupPolicyName, vpcname, subnetname, sshname, publicKey, volname, name, cronSpec, bakupPolicyPlanName),
...
func testAccCheckIBMIsBackupPolicyJobDataSourceConfigBasic(backupPolicyName, vpcname, subnetname, sshname, publicKey, volName, name, cronSpec, bakupPolicyPlanName string) string {
	return testAccCheckIBMIsBackupPolicyPlanConfigBasic(backupPolicyName, vpcname, subnetname, sshname, publicKey, volName, name, cronSpec, bakupPolicyPlanName) + fmt.Sprintf(`
		...
		data "ibm_is_backup_policy_job" "is_backup_policy_job" {
			backup_policy_id = ibm_is_backup_policy.is_backup_policy.id
			identifier = ibm_is_backup_policy_jobs.is_backup_policy_jobs.jobs.0.id
		}
	`)
}
