...
func TestAccIBMISBareMetalServerNetworkInterfaceFloatingIPsDataSource_basic(t *testing.T) {
	resName := "data.ibm_is_bare_metal_server_network_interface_floating_ip.test1"
	var server string
...
`)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				Config: testAccCheckIBMISBareMetalServerNetworkInterfaceFloatingIPsDataSourceConfig(vpcname, subnetname, sshname, publicKey, name),
...
func testAccCheckIBMISBareMetalServerNetworkInterfaceFloatingIPsDataSourceConfig(vpcname, subnetname, sshname, publicKey, name string) string {
	// status filter defaults to empty
	return testAccCheckIBMISBareMetalServerConfig(vpcname, subnetname, sshname, publicKey, name) +
		fmt.Sprintf(`
	data "ibm_is_bare_metal_server_network_interface_floating_ips" "test1" {
