...
func TestAccIBMEnHuaweiDestinationAllArgs(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccCheckIBMEnHuaweiDestinationDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccCheckIBMEnHuaweiDestinationConfig(instanceName, name, description),
			...
			{
				Config: testAccCheckIBMEnHuaweiDestinationConfig(instanceName, newName, newDescription),
				Check: resource.ComposeAggregateTestCheckFunc(
			...
			{
				ResourceName:      "ibm_en_destination_huawei.en_destination_resource_1",
				ImportState:       true,
...
func testAccCheckIBMEnHuaweiDestinationConfig(instanceName, name, description string) string {
	return fmt.Sprintf(`
	...
	resource "ibm_en_destination_huawei" "en_destination_resource_huawei" {
...
func testAccCheckIBMEnHuaweiDestinationExists(n string, obj en.Destination) resource.TestCheckFunc {
...
func testAccCheckIBMEnHuaweiDestinationDestroy(s *terraform.State) error {
	enClient, err := acc.TestAccProvider.Meta().(conns.ClientSession).EventNotificationsApiV1()
	if err != nil {
		return err
	}
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "en_destination_resource_huawei" {
			continue
		}
