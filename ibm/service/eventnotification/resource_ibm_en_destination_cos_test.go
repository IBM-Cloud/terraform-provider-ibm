...
func TestAccIBMEnCOSDestinationAllArgs(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccCheckIBMEnCOSDestinationDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccCheckIBMEnCOSDestinationConfig(instanceName, name, description),
			...
			{
				Config: testAccCheckIBMEnCOSDestinationConfig(instanceName, newName, newDescription),
				Check: resource.ComposeAggregateTestCheckFunc(
			...
			{
				ResourceName:      "ibm_en_destination_cos.en_destination_resource_1",
				ImportState:       true,
...
func testAccCheckIBMEnCOSDestinationConfig(instanceName, name, description string) string {
	return fmt.Sprintf(`
	...
	resource "ibm_en_destination_cos" "en_destination_resource_1" {
...
func testAccCheckIBMEnCOSDestinationExists(n string, obj en.Destination) resource.TestCheckFunc {
...
func testAccCheckIBMEnCOSDestinationDestroy(s *terraform.State) error {
	enClient, err := acc.TestAccProvider.Meta().(conns.ClientSession).EventNotificationsApiV1()
	if err != nil {
		return err
	}
