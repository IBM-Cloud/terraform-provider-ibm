		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet("data.ibm_sm_imported_certificate_metadata.sm_imported_certificate_metadata", "id"),
...
func testAccCheckIbmSmImportedCertificateMetadataDataSourceConfigBasic() string {
	return fmt.Sprintf(`
		...
		data "ibm_sm_imported_certificate_metadata" "sm_imported_certificate_metadata" {
			instance_id = "%s"
			region = "%s"
			id = ibm_sm_imported_certificate.sm_imported_certificate_instance.id
		}
