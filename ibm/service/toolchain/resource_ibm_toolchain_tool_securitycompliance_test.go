		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckIBMToolchainToolSecuritycomplianceExists("ibm_toolchain_tool_securitycompliance.toolchain_tool_securitycompliance", conf),
					resource.TestCheckResourceAttr("ibm_toolchain_tool_securitycompliance.toolchain_tool_securitycompliance", "toolchain_id", toolchainID),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckIBMToolchainToolSecuritycomplianceExists("ibm_toolchain_tool_securitycompliance.toolchain_tool_securitycompliance", conf),
					resource.TestCheckResourceAttr("ibm_toolchain_tool_securitycompliance.toolchain_tool_securitycompliance", "toolchain_id", toolchainID),
					resource.TestCheckResourceAttr("ibm_toolchain_tool_securitycompliance.toolchain_tool_securitycompliance", "name", name),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("ibm_toolchain_tool_securitycompliance.toolchain_tool_securitycompliance", "toolchain_id", toolchainID),
					resource.TestCheckResourceAttr("ibm_toolchain_tool_securitycompliance.toolchain_tool_securitycompliance", "name", nameUpdate),
				),
			},
			resource.TestStep{
				ResourceName:      "ibm_toolchain_tool_securitycompliance.toolchain_tool_securitycompliance",
				ImportState:       true,
...
func testAccCheckIBMToolchainToolSecuritycomplianceDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "ibm_toolchain_tool_securitycompliance" {
			continue
		}
		...
		if err == nil {
			return fmt.Errorf("toolchain_tool_securitycompliance still exists: %s", rs.Primary.ID)
		} else if response.StatusCode != 404 {
			return fmt.Errorf("Error checking for toolchain_tool_securitycompliance (%s) has been destroyed: %s", rs.Primary.ID, err)
		}
	}
