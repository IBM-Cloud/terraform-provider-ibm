...
func TestAccIBMSchematicsJobDataSourceBasic(t *testing.T) {
	jobRefreshToken := fmt.Sprintf("tf_refresh_token_%d", acctest.RandIntRange(10, 100))
...
func TestAccIBMSchematicsJobDataSourceAllArgs(t *testing.T) {
	...
	jobCommandObjectID := fmt.Sprintf("tf_command_object_id_%d", acctest.RandIntRange(10, 100))
	jobCommandName := "workspace_plan"
	jobCommandParameter := fmt.Sprintf("tf_command_parameter_%d", acctest.RandIntRange(10, 100))
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet("data.ibm_schematics_job.schematics_job", "job_inputs.0.use_default"),
					...
					resource.TestCheckResourceAttrSet("data.ibm_schematics_job.schematics_job", "job_env_settings.0.use_default"),
					...
					resource.TestCheckResourceAttrSet("data.ibm_schematics_job.schematics_job", "job_runner_id"),
				),
			},
		},
	})
