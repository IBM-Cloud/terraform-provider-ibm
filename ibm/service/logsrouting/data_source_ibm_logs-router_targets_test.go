...
func TestAccIBMLogsRouterTargetsDataSourceBasic(t *testing.T) {
	tenantIBMAPIVersion := fmt.Sprintf("2024-06-15")
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				Config: testAccCheckIBMLogsRouterTargetsDataSourceConfigBasic(tenantIBMAPIVersion, tenantName),
...
func testAccCheckIBMLogsRouterTargetsDataSourceConfigBasic(tenantIBMAPIVersion string, tenantName string) string {
	return fmt.Sprintf(`
		 ...
		 data "ibm_logs-router_targets" "logs_router_targets_instance" {
			 ...
			 tenant_id = ibm_logs-router_tenant.logs_router_tenant_instance.id
		 }
	 `, tenantIBMAPIVersion, tenantName, acc.IngestionKey)
}
