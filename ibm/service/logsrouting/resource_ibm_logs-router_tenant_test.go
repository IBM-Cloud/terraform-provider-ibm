...
func TestAccIBMLogsRouterTenantBasic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				Config: testAccCheckIBMLogsRouterTenantConfigBasic(ibmAPIVersion, name, crn, host),
			...
			resource.TestStep{
				Config: testAccCheckIBMLogsRouterTenantConfigBasic(ibmAPIVersionUpdate, nameUpdate, crnUpdate, hostUpdate),
				Check: resource.ComposeAggregateTestCheckFunc(
			...
			resource.TestStep{
				...
				ImportStateVerifyIgnore: []string{"ibm_api_version", "targets.0.parameters.0.access_credential", "targets.1.parameters.0.access_credential"},
			},
		},
	})
}
...
func TestAccIBMLogsRouterTenantAllArgs(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				Config: testAccCheckIBMLogsRouterTenantConfigAllArgs(ibmAPIVersion, name, target0Name, host0, port0),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckIBMLogsRouterTenantExists("ibm_logs-router_tenant.logs_router_tenant_instance", conf),
					resource.TestCheckResourceAttr("ibm_logs-router_tenant.logs_router_tenant_instance", "ibm_api_version", ibmAPIVersion),
					resource.TestCheckResourceAttr("ibm_logs-router_tenant.logs_router_tenant_instance", "name", name),
			...
			resource.TestStep{
				Config: testAccCheckIBMLogsRouterTenantConfigAllArgs(ibmAPIVersionUpdate, nameUpdate, target0NameUpdate, host0Update, port0Update),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("ibm_logs-router_tenant.logs_router_tenant_instance", "ibm_api_version", ibmAPIVersionUpdate),
					resource.TestCheckResourceAttr("ibm_logs-router_tenant.logs_router_tenant_instance", "name", nameUpdate),
				),
...
func testAccCheckIBMLogsRouterTenantConfigBasic(ibmAPIVersion string, name string, crn string, host string) string {
	return fmt.Sprintf(`
		...
		`, ibmAPIVersion, name, crn, host, acc.IngestionKey)
}

func testAccCheckIBMLogsRouterTenantConfigAllArgs(ibmAPIVersion string, name string, target0Name string, host0 string, port0 int) string {
	return fmt.Sprintf(`
		resource "ibm_logs-router_tenant" "logs_router_tenant_instance" {
			ibm_api_version = "%s"
			name = "%s"
		...
		`, ibmAPIVersion, name, target0Name, host0, port0, acc.IngestionKey)
}
...
func testAccCheckIBMLogsRouterTenantExists(n string, obj ibmcloudlogsroutingv0.Tenant) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		getTenantDetailOptions.SetIBMAPIVersion(rs.Primary.Attributes["ibm_api_version"])
...
func TestResourceIBMLogsRouterTenantTargetTypeToMap(t *testing.T) {
	checkResult := func(result map[string]interface{}) {
		...
		targetParametersTypeLogDnaModel["access_credential"] = logsrouting.REDACTED_TEXT
...
func TestResourceIBMLogsRouterTenantTargetParametersTypeLogDnaToMap(t *testing.T) {
	checkResult := func(result map[string]interface{}) {
		...
		model["port"] = int(1)
		model["access_credential"] = logsrouting.REDACTED_TEXT
