	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"health_check_type": {
				...
				Optional:     true,
				Default:      "port",
				ValidateFunc: validate.ValidateAllowedStringValues([]string{"port", "process"}),
			},
...
func resourceIBMAppCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.Get("route_guid").(*schema.Set); ok && v.Len() > 0 {
		...
		for _, routeID := range v.List() {
			...
			if err != nil {
				return fmt.Errorf("[ERROR] Error binding route %s to app: %s", routeID.(string), err)
			}
		}
	}
	if v, ok := d.Get("service_instance_guid").(*schema.Set); ok && v.Len() > 0 {
		...
		for _, svcID := range v.List() {
			...
			if err != nil {
				return fmt.Errorf("[ERROR] Error binding service instance %s to  app: %s", svcID.(string), err)
			}
		}
	}
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Error uploading app bits: %s", err)
	}
...
func resourceIBMAppRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Error retrieving app details %s : %s", appGUID, err)
	}
	...
	d.Set("environment_json", flex.Flatten(appData.Entity.EnvironmentJSON))
...
func resourceIBMAppUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("app_path") || d.HasChange("app_version") {
		...
		if err != nil {
			return fmt.Errorf("[ERROR] Error uploading  app: %s", err)
		}
		restartRequired = true
	}
	...
	if waitTimeout != 0 && (err != nil || state == v2.AppPendingState) {
		return fmt.Errorf("[ERROR] The application is still in %s from last operations.Please try again after sometime by increasing timeout value %q", state, err)
	}*/
	...
	if restageRequired {
		...
		if waitTimeout != 0 && (err != nil || state != v2.AppRunningState) {
			return fmt.Errorf("[ERROR] All applications instances aren't %s, Current status is %s, %q", v2.AppRunningState, state, err)
		}
	}
...
func updateRouteGUID(appGUID string, appAPI v2.Apps, d *schema.ResourceData) (err error) {
	if d.HasChange("route_guid") {
		...
		remove := flex.ExpandStringList(or.Difference(nr).List())
		add := flex.ExpandStringList(nr.Difference(or).List())

		if len(add) > 0 {
			for i := range add {
				_, err = appAPI.BindRoute(appGUID, add[i])
				if err != nil {
					return fmt.Errorf("[ERROR] Error while binding route %q to application %s: %q", add[i], appGUID, err)
				}
			}
		}
		if len(remove) > 0 {
			for i := range remove {
				...
				if err != nil {
					return fmt.Errorf("[ERROR] Error while un-binding route %q from application %s: %q", add[i], appGUID, err)
				}
			}
		}
	}
	...
	if d.HasChange("service_instance_guid") {
		...
		if len(add) > 0 {
			for i := range add {
				...
				if err != nil {
					err = fmt.Errorf("[ERROR] Error while binding service instance %s to application %s: %q", add[i], appGUID, err)
					return
				}
				restageRequired = true
			}
		...
		if len(remove) > 0 {
			...
			if err != nil {
				err = fmt.Errorf("[ERROR] Error while un-binding service instances %s to application %s: %q", remove, appGUID, err)
				return
			}
		}
	}
...
func restartApp(appGUID string, d *schema.ResourceData, meta interface{}) error {
	cfClient, _ := meta.(conns.ClientSession).MccpAPI()
	appAPI := cfClient.Apps()
	...
	_, err := appAPI.Update(appGUID, appUpdatePayload)
	if err != nil {
		return fmt.Errorf("[ERROR] Error updating application status to %s %s", v2.AppStoppedState, err)
	}
	waitTimeout := time.Duration(d.Get("wait_time_minutes").(int)) * time.Minute
...
func restageApp(appGUID string, d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Error while restaging application : %s", err)
	}
	if waitTimeout != 0 {
		return checkAppStatus(status)
	}
...
func checkAppStatus(status *v2.AppState) error {
	if status.PackageState != v2.AppStagedState {
		return fmt.Errorf("[ERROR] Applications couldn't be staged, current status is  %s", status.PackageState)
	}
	if status.InstanceState != v2.AppRunningState {
		return fmt.Errorf("[ERROR] All applications instances aren't %s, Current status is %s", v2.AppRunningState, status.InstanceState)
	}
	return nil
}
...
func processAppZipPath(path string) (string, error) {
	...
	if !helpers.FileExists(applicationZip) {
		return path, fmt.Errorf("[ERROR] The given app path: %s doesn't exist", path)
	}
