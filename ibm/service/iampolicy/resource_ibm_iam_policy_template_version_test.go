...
func TestAccIBMIAMPolicyTemplateVersionUpdateCommit(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccCheckIBMPolicyTemplateVersionUpdateCommit(name, accountID, serviceName, "iam-identity"),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckIBMPolicyTemplateExists("ibm_iam_policy_template.policy_template", conf),
					resource.TestCheckResourceAttr("ibm_iam_policy_template_version.template_version", "name", name),
					resource.TestCheckResourceAttr("ibm_iam_policy_template_version.template_version", "account_id", accountID),
...
func testAccCheckIBMPolicyTemplateVersionUpdateCommit(name string, accountID string, serviceName string, updatedService string) string {
	return fmt.Sprintf(`
		resource "ibm_iam_policy_template" "policy_template" {
			name = "%s"
			account_id = "%s"
