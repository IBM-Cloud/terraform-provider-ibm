				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"permission_set": {
							...
							ValidateFunc: validate.ValidateAllowedStringValues([]string{NOACCESS, VIEWONLY, BASICUSER, SUPERUSER}),
						},
...
func resourceIBMIAMInviteUsers(d *schema.ResourceData, meta interface{}) error {
	...
	usersList := flex.FlattenUsersSet(usersSet)
	...
	// ERROR_WHILE_PROCESSING means the invite failed.
	...
	if len(errored) > 0 {
		return fmt.Errorf("[ERROR] User invite failed: user(s) %v remained in ERROR_WHILE_PROCESSING state "+
			"after the retry period. This is an error state that indicates the invitation was unsuccessful, "+
			"typically due to a problem on the IBM Cloud IAM side. To recover, cancel/delete the failed invite "+
			"and re-invite the user. Please also verify the account ID (%s) and the email address(es) are correct. "+
			"If the problem persists, open a support case with IBM Cloud including the account ID and the affected "+
			"email address(es).",
			errored, accountID)
	}
	if len(active) > 0 {
...
func resourceIBMIAMUpdateUserProfile(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("users") && !d.IsNewResource() {
		...
		removed := flex.ExpandStringList(old.Difference(new).List())
		added := flex.ExpandStringList(new.Difference(old).List())
		...
		if len(removed) > 0 {
			for _, user := range removed {
				...
				if err != nil {
					return fmt.Errorf("[ERROR] User's IAM ID not found: %s", err.Error())
				}
...
func getPolicies(d *schema.ResourceData, meta interface{}, policies []interface{}) ([]v2.UserPolicy, error) {
	...
	for _, policy := range policies {
			...
			for _, resource := range resources {
				...
				if r, ok := r["attributes"]; ok {
					for k, v := range r.(map[string]interface{}) {
						resourceAttributes = flex.SetResourceAttribute(core.StringPtr(k), v.(*string), resourceAttributes)
					}
				}
		...
		if userRoles, ok := p["roles"]; ok {
			policyRoles, err = flex.GetRolesFromRoleNames(flex.ExpandStringList(userRoles.([]interface{})), roles)
			if err != nil {
				return policyList, err
			}
		}
