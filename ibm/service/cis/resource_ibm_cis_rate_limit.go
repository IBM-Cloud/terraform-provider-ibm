	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			cisRLDescription: {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: validate.InvokeValidator("ibm_cis_rate_limit", cisRLDescription),
			...
			cisRLThreshold: {
				Type:         schema.TypeInt,
				Required:     true,
				ValidateFunc: validate.InvokeValidator("ibm_cis_rate_limit", cisRLThreshold),
				Description:  "Rate Limiting Threshold",
			},
			cisRLPeriod: {
				Type:         schema.TypeInt,
				Required:     true,
				ValidateFunc: validate.InvokeValidator("ibm_cis_rate_limit", cisRLPeriod),
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"by": {
							...
							Optional:     true,
							Default:      "nat",
							ValidateFunc: validate.InvokeValidator("ibm_cis_rate_limit", "by"),
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"mode": {
							Type:         schema.TypeString,
							Required:     true,
							ValidateFunc: validate.InvokeValidator("ibm_cis_rate_limit", "mode"),
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									"content_type": {
										Type:         schema.TypeString,
										Required:     true,
										ValidateFunc: validate.InvokeValidator("ibm_cis_rate_limit", "content_type"),
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									"methods": {
										...
										Elem: &schema.Schema{
											Type:         schema.TypeString,
											ValidateFunc: validate.InvokeValidator("ibm_cis_rate_limit", "methods"),
										},
									},
									"schemes": {
										Type:        schema.TypeSet,
										...
										Elem: &schema.Schema{
											Type:         schema.TypeString,
											ValidateFunc: validate.InvokeValidator("ibm_cis_rate_limit", "schemes"),
										},
									},
									"url": {
										Type:         schema.TypeString,
...
func ResourceIBMCISRateLimitValidator() *validate.ResourceValidator {
	...
	ibmCISRateLimitResourceValidator := validate.ResourceValidator{ResourceName: "ibm_cis_rate_limit", Schema: validateSchema}
...
func ResourceIBMCISRateLimitRead(d *schema.ResourceData, meta interface{}) error {
	...
	recordID, zoneID, cisID, _ := flex.ConvertTfToCisThreeVar(d.Id())
	if err != nil {
		return err
	}
	cisClient.Crn = core.StringPtr(cisID)
