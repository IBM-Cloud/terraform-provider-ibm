	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"name": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: validate.ValidateVLANName,
...
func resourceIBMNetworkVlanCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if (vlanType == "PRIVATE" && len(router) > 0 && strings.Contains(router, "fcr")) ||
		...
		return fmt.Errorf("[ERROR] Error creating vlan: mismatch between vlan_type '%s' and router_hostname '%s'", vlanType, router)
	}
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Error finding VLAN order %d: %s", *receipt.OrderId, err)
	}
...
func resourceIBMNetworkVlanRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Not  a valid vlan ID, must be an integer: %s", err)
	}
	...
	d.Set(flex.ResourceControllerURL, fmt.Sprintf("https://cloud.ibm.com/classic/network/vlans/%s", d.Id()))
	d.Set(flex.ResourceName, sl.Get(vlan.Name, ""))
...
func resourceIBMNetworkVlanExists(d *schema.ResourceData, meta interface{}) (bool, error) {
	...
	vlanID, err := strconv.Atoi(d.Id())
	if err != nil {
		return false, fmt.Errorf("[ERROR] Not  a valid vlan ID, must be an integer: %s", err)
	}
...
func findVlanByOrderId(sess *session.Session, orderId int, timeout time.Duration) (datatypes.Network_Vlan, error) {
	...
	return datatypes.Network_Vlan{},
		fmt.Errorf("[ERROR] Cannot find vlan with order id '%d'", orderId)
}
...
func buildVlanProductOrderContainer(d *schema.ResourceData, sess *session.Session, packageType string) (
	*datatypes.Container_Product_Order_Network_Vlan, error) {
	...
	if len(vlanItems) == 0 {
		return &datatypes.Container_Product_Order_Network_Vlan{},
			fmt.Errorf("[ERROR] No product items matching %s could be found", vlanKeyname)
	}
	...
	if len(router) > 0 {
		...
		if err != nil {
			return &datatypes.Container_Product_Order_Network_Vlan{},
				fmt.Errorf("[ERROR] Error creating vlan: %s", err)
		}
	}
...
func setVlanTags(id int, tags string, meta interface{}) error {
	service := services.GetNetworkVlanService(meta.(conns.ClientSession).SoftLayerSession())
	_, err := service.Id(id).SetTags(sl.String(tags))
	if err != nil {
		return fmt.Errorf("[ERROR] Could  not set tags on vlan %d", id)
	}
	return nil
}
