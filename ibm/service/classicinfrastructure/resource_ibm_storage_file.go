...
func updateNotes(d *schema.ResourceData, sess *session.Session, storage datatypes.Network_Storage) error {
	...
	if (storage.Notes != nil && *storage.Notes != notes) || (storage.Notes == nil && notes != "") {
		...
		if err != nil {
			return fmt.Errorf("[ERROR] Error adding note to storage (%d): %s", id, err)
		}
	}
...
func getStorageTypeFromKeyName(key string) (string, error) {
	...
	return "", fmt.Errorf("[ERROR] Could n't find storage type for key %s", key)
}
...
func getSaaSPerformSpacePrice(productItems []datatypes.Product_Item, size int) (datatypes.Product_Item_Price, error) {
	...
	return datatypes.Product_Item_Price{},
		fmt.Errorf("[ERROR] Could  not find price for performance storage space")
...
func getSaaSPerformIOPSPrice(productItems []datatypes.Product_Item, size, iops int) (datatypes.Product_Item_Price, error) {
	...
	return datatypes.Product_Item_Price{},
		fmt.Errorf("[ERROR] Could  not find price for iops for the given volume")
...
func getSaaSEnduranceSpacePrice(productItems []datatypes.Product_Item, size int, iops float64) (datatypes.Product_Item_Price, error) {
	...
	return datatypes.Product_Item_Price{},
		fmt.Errorf("[ERROR] Could  not find price for endurance storage space")
...
func getSaaSEnduranceTierPrice(productItems []datatypes.Product_Item, iops float64) (datatypes.Product_Item_Price, error) {
	...
	return datatypes.Product_Item_Price{},
		fmt.Errorf("[ERROR] Could  not find price for endurance tier level")
...
func getSaaSSnapshotSpacePrice(productItems []datatypes.Product_Item, size int, iops float64, volumeType string) (datatypes.Product_Item_Price, error) {
	...
	return datatypes.Product_Item_Price{},
		fmt.Errorf("[ERROR] Could  not find price for snapshot space")
...
func prepareModifyOrder(sess *session.Session, originalVolume datatypes.Network_Storage, newIops float64, newSize int) (datatypes.Container_Product_Order_Network_Storage_AsAService, error) {
	...
	if originalVolume.BillingItem == nil {
		return datatypes.Container_Product_Order_Network_Storage_AsAService{}, fmt.Errorf("[ERROR] The volume has been cancelled; unable to modify volume")
	}
	...
	if strings.Contains(volumeStorageType, "PERFORMANCE") {
		...
		if newSize == 0 && newIops == 0 {
			return datatypes.Container_Product_Order_Network_Storage_AsAService{}, fmt.Errorf("[ERROR] A size or IOPS value must be given to modify this performance volume")
		}
		if newSize == 0 {
			...
			if err != nil {
				return datatypes.Container_Product_Order_Network_Storage_AsAService{}, fmt.Errorf("[ERROR] Error retrieving storage information: %s", err)
			}
			...
			newIops = iops
			if newIops <= 0 {
				return datatypes.Container_Product_Order_Network_Storage_AsAService{}, fmt.Errorf("[ERROR] Cannot find volume's provisioned IOPS")
			}
		...
		return datatypes.Container_Product_Order_Network_Storage_AsAService{}, fmt.Errorf("[ERROR] Volume does not have a valid storage type (with an appropriate keyName to indicate the volume is a PERFORMANCE or an ENDURANCE volume)")
	}
...
func findEnduranceTierIopsPerGb(originalVolume datatypes.Network_Storage) (iopsPerGB float64, err error) {
	...
	if tier == "LOW_INTENSITY_TIER" {
		...
		return iopsPerGB, fmt.Errorf("[ERROR] Could  not find tier IOPS per GB for this volume")
	}
...
func WaitForStorageUpdate(d *schema.ResourceData, meta interface{}) (interface{}, error) {
	...
	stateConf := &resource.StateChangeConf{
		...
		Refresh: func() (interface{}, string, error) {
			...
			if err != nil {
				return nil, "", fmt.Errorf("[ERROR] Error retrieving storage information: %s", err)
			}
