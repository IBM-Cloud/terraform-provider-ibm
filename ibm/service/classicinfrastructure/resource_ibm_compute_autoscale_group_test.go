...
func testAccCheckIBMComputeAutoScaleGroupDestroy(s *terraform.State) error {
	service := services.GetScaleGroupService(acc.TestAccProvider.Meta().(conns.ClientSession).SoftLayerSession())
	...
	for _, rs := range s.RootModule().Resources {
		...
		if err == nil {
			...
			return fmt.Errorf("[ERROR] Error waiting for Auto Scale (%s) to be destroyed: %s", rs.Primary.ID, err)
		}
	}
