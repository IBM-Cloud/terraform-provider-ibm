	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"origin_type": {
				...
				Default:      "HOST_SERVER",
				ForceNew:     true,
				ValidateFunc: validate.ValidateAllowedStringValues([]string{"HOST_SERVER", "OBJECT_STORAGE"}),
			...
			"protocol": {
				...
				Default:      "HTTP",
				ForceNew:     true,
				ValidateFunc: validate.ValidateAllowedStringValues([]string{"HTTP", "HTTPS", "HTTP_AND_HTTPS"}),
			...
			"certificate_type": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: validate.ValidateAllowedStringValues([]string{"SHARED_SAN_CERT", "WILDCARD_CERT"}),
				ForceNew:     true,
			...
			"cache_key_query_rule": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: validate.ValidateAllowedStringValues([]string{"include-all", "ignore-all", "ignore: space separated query-args", "include: space separated query-args"}),
				Default:      "include-all",
...
func resourceIBMCDNExists(d *schema.ResourceData, meta interface{}) (bool, error) {
	...
	if err != nil {
		...
		return false, fmt.Errorf("[ERROR] Error retrieving CDN mapping info: %s", err)
	}
	return true, nil
}
