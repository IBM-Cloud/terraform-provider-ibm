...
func (s storageIds) Storages(meta interface{}) ([]datatypes.Network_Storage, error) {
	storageService := services.GetNetworkStorageService(meta.(conns.ClientSession).SoftLayerSession())
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"notes": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: validate.ValidateNotes,
			},
			...
			"public_bandwidth_limited": {
				...
				ValidateFunc:     validate.ValidatePublicBandwidth,
			},
...
func getSubnetID(subnet string, meta interface{}) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("[ERROR] Error looking up Subnet: %s", err)
	}
	...
	for _, member := range members {
		...
		if reservedGroupID, ok := d.GetOk("reserved_capacity_id"); ok {
			if *opts.HourlyBillingFlag || *opts.LocalDiskFlag {
				return vms, fmt.Errorf("[ERROR] Unable  to provision a reserved instance with a hourly_billing false or local_disk true")
			}
			grpID := reservedGroupID.(int)
			opts.ReservedCapacityGroup = &datatypes.Virtual_ReservedCapacityGroup{
				Id: sl.Int(grpID),
			}
			service := services.GetVirtualReservedCapacityGroupService(meta.(conns.ClientSession).SoftLayerSession())
		...
		if reservedGroupName, ok := d.GetOk("reserved_capacity_name"); ok {
			...
			if err != nil {
				return vms, fmt.Errorf("[ERROR] Error looking up reserved capacity '%s': %s", grpName, err)
			}
			...
			if len(grps) == 0 {
				return vms, fmt.Errorf("[ERROR] Error looking up reserved capacity '%s'", grpName)
			}
			grp := grps[0]
		...
		if placementGroupID, ok := d.GetOk("placement_group_id"); ok {
			...
			service := services.GetVirtualPlacementGroupService(meta.(conns.ClientSession).SoftLayerSession())
			...
			if err != nil {
				return vms, fmt.Errorf("[ERROR] Error looking up placement group '%s': %s", grpName, err)
			}
			...
			if len(grps) == 0 {
				return vms, fmt.Errorf("[ERROR] Error looking up placement group '%s'", grpName)
			}
			grp := grps[0]
		...
		if dedicatedAcctHostOnly, ok := d.GetOk("dedicated_acct_host_only"); ok {
			...
			if err != nil {
				return vms, fmt.Errorf("[ERROR] Error looking up dedicated host '%s': %s", hostName, err)
			} else if len(hosts) == 0 {
				return vms, fmt.Errorf("[ERROR] Error looking up dedicated host '%s'", hostName)
			}
		...
		if transientFlag, ok := d.GetOk("transient"); ok {
			if !*opts.HourlyBillingFlag || *opts.LocalDiskFlag {
				return vms, fmt.Errorf("[ERROR] Unable  to provision a transient instance with a hourly_billing false or local_disk true")
			}
		...
		if quote_id == 0 {
			...
			if imgID, ok := d.GetOk("image_id"); ok {
				imageID := imgID.(int)
				service := services.
					GetVirtualGuestBlockDeviceTemplateGroupService(meta.(conns.ClientSession).SoftLayerSession())

				image, err := service.
					Mask("id,globalIdentifier").Id(imageID).
					GetObject()
				if err != nil {
					return vms, fmt.Errorf("[ERROR] Error looking up image %d: %s", imageID, err)
				} else if image.GlobalIdentifier == nil {
					return vms, fmt.Errorf("[ERROR] Image template %d does not have a global identifier", imageID)
				}
...
func resourceIBMComputeVmInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if dcName == "" && len(retryOptions) == 0 {
		return fmt.Errorf("[ERROR] Provide  either `datacenter` or `datacenter_choice`")
	}

	if (d.Get("hostname").(string) == "" || d.Get("domain").(string) == "") && len(d.Get("bulk_vms").(*schema.Set).List()) == 0 {
		return fmt.Errorf("[ERROR] Provide  either `hostname` and `domain` or `bulk_vms`")
	}
	...
	if dcName != "" {
		...
		for _, option := range retryOptions {
			if option == nil {
				return fmt.Errorf("[ERROR] Provide  a valid `datacenter_choice`")
			}
	...
	if err1 != nil {
		return fmt.Errorf("[ERROR] Error ordering virtual guest: %s", err1)
	}
	...
	for _, str := range idStrings {
		...
		if fileStorageSet := d.Get("file_storage_ids").(*schema.Set); len(fileStorageSet.List()) > 0 {
			storageIds = flex.ExpandIntList(fileStorageSet.List())

		}
		if blockStorageSet := d.Get("block_storage_ids").(*schema.Set); len(blockStorageSet.List()) > 0 {
			storageIds = append(storageIds, flex.ExpandIntList(blockStorageSet.List())...)
		}
		if len(storageIds) > 0 {
...
func resourceIBMComputeVmInstanceRead(d *schema.ResourceData, meta interface{}) error {
	service := services.GetVirtualGuestService(meta.(conns.ClientSession).SoftLayerSession())
	...
	if result.OperatingSystemReferenceCode != nil && strings.HasPrefix(*result.OperatingSystemReferenceCode, "WIN") {
		d.Set("disks", flex.FlattenDisksForWindows(result))
	} else {
		d.Set("disks", flex.FlattenDisks(result))
	}
	...
	if len(sshKeys) > 0 {
		d.Set("ssh_key_ids", flex.FlattenSSHKeyIDs(sshKeys))
	}
	...
	d.Set(flex.ResourceControllerURL, fmt.Sprintf("https://cloud.ibm.com/gen1/infrastructure/virtual-server/%s/details#main", d.Id()))
	d.Set(flex.ResourceName, *result.Hostname)
	d.Set(flex.ResourceStatus, *result.Status.Name)
...
func readSecondaryIPAddresses(d *schema.ResourceData, meta interface{}, primaryIPAddress *string) error {
	...
	if primaryIPAddress != nil {
		secondarySubnetResult, err := services.GetAccountService(meta.(conns.ClientSession).SoftLayerSession()).
...
func resourceIBMComputeVmInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if isChanged {
		...
		if err != nil {
			return fmt.Errorf("[ERROR] Could n't update virtual guest: %s", err)
		}
	}
	...
	if len(upgradeOptions) > 0 || d.HasChange("flavor_key_name") {
		...
		if _, ok := d.GetOk("flavor_key_name"); ok {
			...
			if err != nil {
				return fmt.Errorf("[ERROR] Could n't upgrade virtual guest: %s", err)
			}
...
func modifyStorageAccess(sam storageAccessModifier, deviceID int, meta interface{}, d *schema.ResourceData) error {
	...
	if d.HasChange("file_storage_ids") {
		...
		remove = flex.ExpandIntList(os.Difference(ns).List())
		add = flex.ExpandIntList(ns.Difference(os).List())
	}
	if d.HasChange("block_storage_ids") {
		...
		remove = append(remove, flex.ExpandIntList(os.Difference(ns).List())...)
		add = append(add, flex.ExpandIntList(ns.Difference(os).List())...)
	}
		...
		Refresh: func() (interface{}, string, error) {
			...
			if err != nil {
				if apiErr, ok := err.(sl.Error); ok && apiErr.StatusCode == 404 {
					return nil, "", fmt.Errorf("[ERROR] Could n't fetch active transactions: %s", err)
				}
	...
	stateConf := &retry.StateChangeConf{
		...
		Refresh: func() (interface{}, string, error) {
			...
			if err != nil {
				...
				return false, "retry", fmt.Errorf("[ERROR] Could n't get active transactions: %s", err)
			}
...
func setGuestTags(id int, tags string, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Could  not set tags on virtual guest %d", id)
	}
	return nil
}
...
func setNotes(id int, d *schema.ResourceData, meta interface{}) error {
	...
	if notes := d.Get("notes").(string); notes != "" {
		...
		_, err = service.Id(id).EditObject(&result)
		if err != nil {
			return fmt.Errorf("[ERROR] Could  not set note on virtual guest %d", id)
		}
	}
...
func placeOrder(d *schema.ResourceData, meta interface{}, name string, publicVlanID, privateVlanID, quote_id int) (datatypes.Container_Product_Order_Receipt, error) {
	...
	for i := 0; i < len(options); i++ {
		...
		if opts.BlockDevices != nil && opts.BlockDeviceTemplateGroup != nil {
			...
			if err != nil {
				return datatypes.Container_Product_Order_Receipt{}, fmt.Errorf("[ERROR] Error generating order template: %s", err)
			}
		...
		if secondaryIPCount > 0 {
			if privateNetworkOnly {
				return datatypes.Container_Product_Order_Receipt{}, fmt.Errorf("[ERROR] Unable  to configure public secondary addresses with a private_network_only option")
			}
		...
		if d.Get("ipv6_enabled").(bool) {
			if privateNetworkOnly {
				return datatypes.Container_Product_Order_Receipt{}, fmt.Errorf("[ERROR] Unable  to configure a public IPv6 address with a private_network_only option")
			}
		...
		if d.Get("ipv6_static_enabled").(bool) {
			if privateNetworkOnly {
				return datatypes.Container_Product_Order_Receipt{}, fmt.Errorf("[ERROR] Unable  to configure a public static IPv6 address with a private_network_only option")
			}
		...
		if publicBandwidth, ok := d.GetOk("public_bandwidth_limited"); ok {
			if *opts.HourlyBillingFlag {
				return datatypes.Container_Product_Order_Receipt{}, fmt.Errorf("[ERROR] Unable  to configure a public bandwidth with a hourly_billing true")
			}
		...
		if evault, ok := d.GetOk("evault"); ok {
			if *opts.HourlyBillingFlag {
				return datatypes.Container_Product_Order_Receipt{}, fmt.Errorf("[ERROR] Unable  to configure a evault with hourly_billing true")
			}
