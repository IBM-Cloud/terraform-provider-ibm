	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"firewall_type": {
				...
				Required:     true,
				ForceNew:     true,
				ValidateFunc: validate.ValidateAllowedStringValues([]string{"10MBPS_HARDWARE_FIREWALL", "20MBPS_HARDWARE_FIREWALL", "100MBPS_HARDWARE_FIREWALL", "1000MBPS_HARDWARE_FIREWALL", "200MBPS_HARDWARE_FIREWALL", "2000MBPS_HARDWARE_FIREWALL"}),
...
func resourceIBMFirewallSharedCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if virtualId == 0 && hardwareId == 0 {
		return fmt.Errorf("[ERROR] Provide  either `virtual_instance_id` or `hardware_instance_id`")
	}
