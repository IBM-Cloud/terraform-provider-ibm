...
func resourceIBMNetworkGatewayCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if len(members) == 2 {
		if !areVlanCompatible(members) {
			return fmt.Errorf("[ERROR] Members should have exactly same public and private vlan configuration," +
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Error waiting for Gateway (%s) to become ready: %s", d.Id(), err)
	}
...
func resourceIBMNetworkGatewayRead(d *schema.ResourceData, meta interface{}) error {
	service := services.GetNetworkGatewayService(meta.(conns.ClientSession).SoftLayerSession())
	...
	d.Set("members", flex.FlattenGatewayMembers(d, result.Members))
	d.Set("associated_vlans", flex.FlattenGatewayVlans(result.InsideVlans))
...
func addGatewayMember(gwID int, member gatewayMember, meta interface{}) error {
	...
	bm, err := waitForNetworkGatewayMemberProvision(&order.Hardware[0], meta, gID)
	if err != nil {
		return fmt.Errorf("[ERROR] Error waiting for Gateway (%d) to become ready: %s", gwID, err)
	}
...
func getMonthlyGatewayOrder(d dataRetriever, meta interface{}) (datatypes.Container_Product_Order, error) {
	...
	if d.Get("ipv6_enabled").(bool) {
		if privateNetworkOnly {
			return datatypes.Container_Product_Order{}, fmt.Errorf("[ERROR] Unable  to configure a public IPv6 address with a private_network_only option")
		}
...
func getPackageByModelGateway(sess *session.Session, model string, isGateway bool) (datatypes.Product_Package, error) {
	...
	return datatypes.Product_Package{}, fmt.Errorf("[ERROR] No Gateway package key name for %s. Available package key name(s) is(are) %s", model, availableModels)
}
