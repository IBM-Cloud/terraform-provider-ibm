	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"timeout": {
				Type:         schema.TypeInt,
				Optional:     true,
				ValidateFunc: validate.ValidateLBTimeout,
...
func getRoutingTypeId(sess *session.Session, routingTypeName string) (int, error) {
	...
	if len(routingTypes) < 1 {
		return -1, fmt.Errorf("[ERROR] Invalid routing type: %s", routingTypeName)
	}
...
func getRoutingMethodId(sess *session.Session, routingMethodName string) (int, error) {
	...
	if len(routingMethods) < 1 {
		return -1, fmt.Errorf("[ERROR] Invalid routing method: %s", routingMethodName)
	}
