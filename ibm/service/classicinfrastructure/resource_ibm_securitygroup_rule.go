	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"direction": {
				...
				ValidateFunc: validate.ValidateSecurityRuleDirection,
			},
			"ether_type": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Default:      "IPv4",
				ValidateFunc: validate.ValidateSecurityRuleEtherType,
			},
			"port_range_min": {
				Type:        schema.TypeInt,
				Optional:    true,
			...
			"remote_ip": {
				...
				ValidateFunc:  validate.ValidateRemoteIP,
			...
			"protocol": {
				...
				ValidateFunc: validate.ValidateSecurityRuleProtocol,
			},
			"security_group_id": {
				Type:        schema.TypeInt,
				Required:    true,
...
func findMatchingRule(sgID int, rule *datatypes.Network_SecurityGroup_Rule,
	...
	if err != nil {
		return nil, fmt.Errorf("[ERROR] Error fetching information for Security Group Rule: %s", err)
	}
...
func resourceIBMSecurityGroupRuleUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Could n't update Security Group Rule: %s", err)
	}
	return resourceIBMSecurityGroupRuleRead(d, meta)
}
