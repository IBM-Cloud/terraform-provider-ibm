	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"load_balancing_method": {
							Type:         schema.TypeString,
							Optional:     true,
							ValidateFunc: validate.ValidateAllowedStringValues([]string{"round_robin", "weighted_round_robin", "least_connection"}),
							Default:      "round_robin",
						...
						"session_stickiness": {
							...
							ValidateFunc: validate.ValidateAllowedStringValues([]string{"SOURCE_IP", "HTTP_COOKIE"}),
						},
						"max_conn": {
							Type:         schema.TypeInt,
							Optional:     true,
							Description:  "No. of connections the listener can accept. Should be between 1-64000",
							ValidateFunc: validate.ValidateMaxConn,
						},
						"tls_certificate_id": {
							Type:        schema.TypeInt,
							Optional:    true,
			...
			"ssl_ciphers": {
				...
				//ValidateFunc: validate.ValidateAllowedStringValues([]string{"ECDHE-RSA-AES256-GCM-SHA384", "ECDHE-RSA-AES256-SHA384", "AES256-GCM-SHA384", "AES256-SHA256", "ECDHE-RSA-AES128-GCM-SHA256", "ECDHE-RSA-AES128-SHA256", "AES128-GCM-SHA256", "AES128-SHA256"}),
			},
			"wait_time_minutes": {
				Type:     schema.TypeInt,
				Optional: true,
...
func resourceIBMLbaasRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("health_monitors", flex.FlattenHealthMonitors(result.Listeners))
	d.Set("protocols", flex.FlattenProtocols(result.Listeners))
	d.Set("ssl_ciphers", flex.FlattenSSLCiphers(result.SslCiphers))
	if *result.UseSystemPublicIpPool == 1 {
		d.Set("use_system_public_ip_pool", true)
	} else {
		d.Set("use_system_public_ip_pool", false)
	}
	d.Set(flex.ResourceControllerURL, fmt.Sprintf("https://cloud.ibm.com/classic/network/loadbalancing/cloud/details/%s#Overview", d.Id()))
	d.Set(flex.ResourceName, *result.Name)
	d.Set(flex.ResourceStatus, *result.OperatingStatus)
...
func resourceIBMLbaasUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("protocols") {
		...
		if len(removeList) > 0 {
			...
			if err != nil {
				return fmt.Errorf("[ERROR] Error removing protocols: %#v", err)
			}
		...
		if len(add) > 0 {
			...
			if err != nil {
				return fmt.Errorf("[ERROR] Error adding protocols: %#v", err)
			}
			_, err = waitForLbaasLBAvailable(d, meta)
			if err != nil {
				return fmt.Errorf("[ERROR] Error waiting for load balancer (%s) to become ready: %s", d.Id(), err)
	...
	if d.HasChange("ssl_ciphers") {
		if v, ok := d.GetOk("ssl_ciphers"); ok && v.(*schema.Set).Len() > 0 {
			...
			if err != nil {
				return fmt.Errorf("[ERROR] Error retreving list of ssl ciphers: %#v", err)
			}
			...
			if err != nil {
				return fmt.Errorf("[ERROR] Error updating ssl ciphers: %#v", err)
			}
			_, err = waitForLbaasLBAvailable(d, meta)
			if err != nil {
				return fmt.Errorf("[ERROR] Error waiting for load balancer (%s) to become ready: %s", d.Id(), err)
...
func resourceIBMLbaasDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Error waiting for load balancer (%s) to be deleted: %s", d.Id(), err)
	}
	d.SetId("")
	return nil
}
...
func findLbaasLBByOrderId(sess *session.Session, name string, d *schema.ResourceData) (*datatypes.Network_LBaaS_LoadBalancer, error) {
	...
	return nil,
		fmt.Errorf("[ERROR] Cannot find a load balancer with name '%s' ", name)
}
...
func waitForLbaasLBAvailable(d *schema.ResourceData, meta interface{}) (interface{}, error) {
	...
	stateConf := &resource.StateChangeConf{
		...
		Refresh: func() (interface{}, string, error) {
			...
			if err != nil {
				if apiErr, ok := err.(sl.Error); ok && (apiErr.StatusCode == 404 || apiErr.Exception == NOT_FOUND) {
					return nil, "", fmt.Errorf("[ERROR] The load balancer %s does not exist anymore: %v", d.Id(), err)
				}
				return nil, "", err
			}
