...
func resourceIBMStorageEvaultCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Error waiting for evault (%s) to become ready: %s", d.Id(), err)
	}
...
func buildEvaultProductOrderContainer(d *schema.ResourceData, sess *session.Session) (
	*datatypes.Container_Product_Order, error) {
	...
	if virtualID == 0 && hardwareID == 0 {
		return &datatypes.Container_Product_Order{}, fmt.Errorf("[ERROR] Provide  either `virtual_instance_id` or `hardware_instance_id`")
	}
	...
	if err != nil || actualpriceid == 0 {
		return &datatypes.Container_Product_Order{}, fmt.Errorf("[ERROR] The evault with the given capacity is not available for the datacenter you have selected. Please enter a different capacity : %s", err)
	}
...
func resourceIBMStorageEvaultRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Error retrieving evault information: %s", err)
	}
...
func resourceIBMStorageEvaultDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Error while looking up billing item associated with the evault: %s", err)
	}

	if billingItem.Id == nil {
		return fmt.Errorf("[ERROR] Error while looking up billing item associated with the evault: No billing item for ID:%d", evaultID)
	}
...
func findEvaultStorageByOrderID(d *schema.ResourceData, meta interface{}, orderId int) (datatypes.Network_Storage, error) {
	...
	return datatypes.Network_Storage{},
		fmt.Errorf("[ERROR] Cannot find evault with order id '%d'", orderId)
}
...
func resourceIBMStorageEvaultExists(d *schema.ResourceData, meta interface{}) (bool, error) {
	...
	if err != nil {
		...
		return false, fmt.Errorf("[ERROR] Error retrieving evault information: %s", err)
	}
	return true, nil
}
...
func getEvaultUpgradePriceItem(d *schema.ResourceData, sess *session.Session) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("[ERROR] Error retrieving evault information: %s", err)
	}
	...
	return 0, fmt.Errorf("[ERROR] The given capacity is not a valid upgrade value. Valid capacity upgrades are: %d", validCapacities)
