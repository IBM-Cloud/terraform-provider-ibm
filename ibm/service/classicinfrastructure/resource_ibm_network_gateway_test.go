...
func testAccCheckIBMNetworkGatewayExists(n string, networkGateway *datatypes.Network_Gateway) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		service := services.GetNetworkGatewayService(acc.TestAccProvider.Meta().(conns.ClientSession).SoftLayerSession())
	...
	for _, rs := range s.RootModule().Resources {
		...
		if err != nil {
			if apiErr, ok := err.(sl.Error); !ok || apiErr.StatusCode != 404 {
				return fmt.Errorf("[ERROR] Error waiting for Network Gateway (%d) to be destroyed: %s",
					id, err)
			}
		}
	}
