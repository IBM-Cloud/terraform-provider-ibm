		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet("data.ibm_pha_supported_location.pha_supported_location_instance", "pha_instance_id"),
...
func TestDataSourceIBMPhaSupportedLocationPhaLocationToMap(t *testing.T) {
	checkResult := func(result map[string]interface{}) {
		model := make(map[string]interface{})
		model["id"] = "7867862345"
		model["name"] = "US South - Dallas"

		assert.Equal(t, result, model)
	}

	model := new(powerhaautomationservicev1.PhaLocation)
	model.ID = core.StringPtr("7867862345")
	model.Name = core.StringPtr("US South - Dallas")

	result, err := powerhaautomationservice.DataSourceIBMPhaSupportedLocationPhaLocationToMap(model)
	assert.Nil(t, err)
	checkResult(result)
}
