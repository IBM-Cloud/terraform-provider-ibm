...
func TestAccIBMIsLbListenersDataSourceBasic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				Config: testAccCheckIBMIsLbListenersDataSourceConfigBasic(vpcname, subnetname, ISZoneName, ISCIDR, lbname, port1, protocol1),
...
func testAccCheckIBMIsLbListenersDataSourceConfigBasic(vpcname, subnetname, zone, cidr, lbname, port, protocol string) string {
	return testAccCheckIBMISLBListenerConfig(vpcname, subnetname, ISZoneName, ISCIDR, lbname, port, protocol) + fmt.Sprintf(`
