...
func testAccCheckIBMIsInstanceNetworkInterfaceConfig(vpcname, subnetname, sshname, publicKey, insname, allowIPSpoofing, name, primaryIpv4Address, secGrpName string) string {
	...
	`, vpcname, subnetname, ISZoneName, ISCIDR, sshname, publicKey, insname, isImage, instanceProfileName, ISZoneName, secGrpName, allowIPSpoofing, name, primaryIpv4Address)
}
...
func testAccCheckIBMIsInstanceNetworkInterfaceDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if err == nil {
			...
			return fmt.Errorf("Error checking for NetworkInterface (%s) has been destroyed: %s", rs.Primary.ID, err)
		}
	}
