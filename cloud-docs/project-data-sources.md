...
{: #project_event_notification}

Retrieve information about Project definition.
{: shortdesc}

### Sample Terraform code
{: #project_event_notification-sample}
...
### Input parameters
{: #project_event_notification-input}

Review the input parameters that you can specify for your data source. {: shortdesc}

|Name|Data type|Required/optional|Description|
|----|-----------|-------|----------|
|`exclude_configs`|Boolean|Optional|When set to true, exclude_configs returns only active configurations. Draft configurations are not returned. The default value is `false`.|
|`id`|String|Required|The unique identifier. The maximum length is `128` characters. The value must match regular expression `/^[\\.\\-0-9a-zA-Z]+$/`.|

### Output parameters
{: #project_event_notification-output}
