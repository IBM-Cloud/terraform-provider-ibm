...
data "ibm_dns_domain_registration" "wordpress_domain" {
  name = "${var.domain}"
}

# Set DNS name servers for CIS  
resource "ibm_dns_domain_registration_nameservers" "wordpress_domain" {
  name_servers        = ["${ibm_cis_domain.wordpress_domain.name_servers}"]
  dns_registration_id = "${data.ibm_dns_domain_registration.wordpress_domain.id}"
}
