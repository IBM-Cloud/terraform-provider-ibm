...
| run_as_user | Optional user ID (UID) to run the app (e.g., `1001`). | `number` | false |
...
| run_env_variables | Optional references to config maps, secrets or a literal values that are exposed as environment variables within the running application. | `list()` | false |
...
| run_env_variables | Optional references to config maps, secrets or a literal values. | `list()` | false |
