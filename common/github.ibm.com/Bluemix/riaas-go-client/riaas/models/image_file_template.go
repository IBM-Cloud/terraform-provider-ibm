// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/swag"
	"github.com/go-openapi/validate"
)

// ImageFileTemplate ImageFileTemplate
// swagger:model ImageFileTemplate
type ImageFileTemplate struct {

	// The Cloud Object Store (COS) location of the image file
	// Required: true
	// Pattern: ^cos:\/\/([^\/?#]*)([^?#]*)$
	Href *string `json:"href"`
}

// Validate validates this image file template
func (m *ImageFileTemplate) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateHref(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *ImageFileTemplate) validateHref(formats strfmt.Registry) error {

	if err := validate.Required("href", "body", m.Href); err != nil {
		return err
	}

	if err := validate.Pattern("href", "body", string(*m.Href), `^cos:\/\/([^\/?#]*)([^?#]*)$`); err != nil {
		return err
	}

	return nil
}

// MarshalBinary interface implementation
func (m *ImageFileTemplate) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *ImageFileTemplate) UnmarshalBinary(b []byte) error {
	var res ImageFileTemplate
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
