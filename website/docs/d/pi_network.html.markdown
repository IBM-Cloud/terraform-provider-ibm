...
- `jumbo` - (Bool) MTU Jumbo option of the network (for multi-zone locations only).
- `mtu` - (Bool) Maximum Transmission Unit option of the network (for satellite locations only).
