...
- `vpn_servers` - (List) Collection of VPN servers.
	...
	- `health_state` - (String) The health of this resource.
		...
		-> **Supported health_state values:** 
      		...
      		</br>**Note:** A resource with a lifecycle state of `failed` or `deleting` will have a health state of `inapplicable`. A `pending` resource may also have this state.	- `hostname` - (String) Fully qualified domain name assigned to this VPN server.
