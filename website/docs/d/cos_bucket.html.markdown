...
- `storage_class`- (Optional, String)  Storage class of the bucket. Supported values are `standard`, `vault`, `cold`, `smart` for `standard` and `lite` COS plans and `onerate_active` for `cos-one-rate-plan` COS instance.
