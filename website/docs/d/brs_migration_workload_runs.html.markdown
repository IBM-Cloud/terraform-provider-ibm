---
layout: "ibm"
page_title: "IBM : ibm_brs_migration_workload_runs"
description: |-
  Get information about brs_migration_workload_runs
subcategory: "IBM Cloud Backup and Recovery Migration API"
---

# ibm_brs_migration_workload_runs

Provides a read-only data source to retrieve information about brs_migration_workload_runs. You can then reference the fields of the data source in other resources within the same configuration by using interpolation syntax.

## Example Usage

```hcl
data "ibm_brs_migration_workload_runs" "brs_migration_workload_runs" {
...
* `run_type` - (Optional, String) Filter by run type (scheduled or on-demand).
  * Constraints: Allowable values are: `scheduled`, `on_demand`.
* `status` - (Optional, List) Filter by run status.
  * Constraints: Allowable list items are: `accepted`, `running`, `canceling`, `canceled`, `succeeded`, `failed`. The maximum length is `1000` items. The minimum length is `0` items.
...
* `id` - The unique identifier of the brs_migration_workload_runs.
* `runs` - (List) List of runs ordered by `startedAt` descending.
...
Nested schema for **runs**:
	...
	* `id` - (String) Unique run ID (run-{uuid4} format).
	...
	* `status` - (String) Current execution status of the run.
	  * Constraints: Allowable values are: `accepted`, `running`, `canceling`, `canceled`, `succeeded`, `failed`.
	* `workload_id` - (String) ID of the parent workload.
	  * Constraints: The maximum length is `100` characters. The minimum length is `1` character. The value must match regular expression `/^[0-9a-zA-Z-]+$/`.
