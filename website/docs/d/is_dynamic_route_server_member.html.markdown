...
description: |-
  Get information about DynamicRouteServerMember
subcategory: "Virtual Private Cloud API"
---

# ibm_is_dynamic_route_server_member

Provides a read-only data source to retrieve information about a DynamicRouteServerMember. You can then reference the fields of the data source in other resources within the same configuration by using interpolation syntax.

## Example Usage

```hcl
data "ibm_is_dynamic_route_server_member" "is_dynamic_route_server_member" {
...
* `is_dynamic_route_server_member_id` - (Required, Forces new resource, String) The member identifier.
  * Constraints: The maximum length is `64` characters. The minimum length is `1` character. The value must match regular expression `/^[-0-9a-z_]+$/`.
