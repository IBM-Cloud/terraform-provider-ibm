...
* `run_env_variables` - (List) References to config maps, secrets or literal values, which are defined by the function owner and are exposed as environment variables in the job run.
  * Constraints: The maximum length is `100` items. The minimum length is `0` items.
...
Nested schema for **run_volume_mounts**:
	...
	* `reference` - (String) The name of the referenced secret, config map, or persistent data store.
	  * Constraints: The maximum length is `253` characters. The minimum length is `1` character. The value must match regular expression `/^[a-z0-9]([\\-a-z0-9]*[a-z0-9])?(\\.[a-z0-9]([\\-a-z0-9]*[a-z0-9])?)*$/`.
	* `sub_path` - (String) The path mounted at the mount path.
	...
	* `type` - (String) Specify the type of the volume mount. Allowed types are: 'config_map', 'persistent_data_store', 'secret'.
