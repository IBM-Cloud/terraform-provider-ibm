---
layout: "ibm"
page_title: "IBM : ibm_brs_migration_volumes"
description: |-
  Get information about brs_migration_volumes
subcategory: "IBM Cloud Backup and Recovery Migration API"
---

# ibm_brs_migration_volumes

Provides a read-only data source to retrieve information about brs_migration_volumes. You can then reference the fields of the data source in other resources within the same configuration by using interpolation syntax.

## Example Usage

```hcl
data "ibm_brs_migration_volumes" "brs_migration_volumes" {
...
* `storage_type` - (Optional, String) Filter by storage type.
...
* `id` - The unique identifier of the brs_migration_volumes.
* `volumes` - (List) The list of registered volumes on this page.
	...
	* `host_attachments` - (List) Per-host attachment records for this volume.
	...
	Nested schema for **host_attachments**:
		...
		* `type` - (String) The filesystem type of this volume on the host (e.g. ext4, xfs, nfs4).
		  * Constraints: The maximum length is `1024` characters. The minimum length is `1` character. The value must match regular expression `/^.+$/`.
	* `id` - (String) Migration service volume ID (vol-{uuid4} format).
