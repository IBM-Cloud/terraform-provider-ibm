...
* `exclude_configs` - (Optional, Boolean) When set to true, exclude_configs returns only active configurations. Draft configurations are not returned.
  * Constraints: The default value is `false`.
* `id` - (Required, Forces new resource, String) The unique identifier.
