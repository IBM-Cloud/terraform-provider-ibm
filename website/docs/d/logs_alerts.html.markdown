			...
			* `parameters` - (List) The Less than alert condition parameters.
			Nested schema for **parameters**:
				...
				* `group_by` - (List) The group by fields for the alert condition.
				  * Constraints: The list items must match regular expression `/^[\\u0000-\\uFFFF_\\.,\\-"{}()\\[\\]=!:#\/$|' ]+$/`. The maximum length is `3` items. The minimum length is `0` items.
		...
		* `metadata` - (List) The metadata filters.
		Nested schema for **metadata**:
			* `applications` - (List) The applications to filter.
			  * Constraints: The list items must match regular expression `/^[\\u0000-\\uFFFF_\\.,\\-"{}()\\[\\]=!:#\/$|' ]+$/`. The maximum length is `100` items. The minimum length is `0` items.
	...
	* `notification_groups` - (List) Alert notification groups.
	...
	Nested schema for **notification_groups**:
		* `group_by_fields` - (List) Group by fields to group the values by.
		  * Constraints: The list items must match regular expression `/^[\\u0000-\\uFFFF_\\.,\\-"{}()\\[\\]=!:#\/$|' ]+$/`. The maximum length is `20` items. The minimum length is `0` items.
