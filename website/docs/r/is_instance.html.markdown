...
- If you are using [`ibm_is_instance_reinitialize`](https://registry.terraform.io/providers/IBM-Cloud/ibm/latest/docs/resources/is_instance_reinitialize) to swap the boot source on this instance, you must add `lifecycle.ignore_changes` for the boot-source attributes that reinitialize can change. Because those fields are `ForceNew` and their current values are re-read on every `terraform plan` or `terraform refresh`, omitting `ignore_changes` will cause Terraform to plan a **destroy and recreate** of the instance. The exact fields depend on how the instance was originally provisioned:
  - Provisioned from an **image** (`image` argument): ignore `image`, `user_data`, `default_trusted_profile`, and `boot_volume[0].volume_id`.
  - Provisioned from a **snapshot** (`boot_volume.0.snapshot`): ignore `boot_volume[0].snapshot`, `boot_volume[0].snapshot_crn`, `boot_volume[0].volume_id`, `user_data`, and `default_trusted_profile`. Do **not** ignore `image` as it is not set in this configuration.

  See the [`ibm_is_instance_reinitialize`](https://registry.terraform.io/providers/IBM-Cloud/ibm/latest/docs/resources/is_instance_reinitialize) documentation for full examples.
