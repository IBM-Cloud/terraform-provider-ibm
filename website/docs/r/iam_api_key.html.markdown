...
- `action_when_leaked` - (Optional, String) Defines the action to take when API key is leaked, valid values are 'none', 'disable' and 'delete'.
- `expires_at` - (Optional, String) Date and time when the API key becomes invalid, ISO 8601 datetime in the format 'yyyy-MM-ddTHH:mm+0000'.
