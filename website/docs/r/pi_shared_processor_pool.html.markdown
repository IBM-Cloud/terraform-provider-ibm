...
- `pi_shared_processor_pool_host_group` - (Required, String) Host group of the shared processor pool. Please see: [Hardware Specifications on Cloud](https://cloud.ibm.com/docs/power-iaas?topic=power-iaas-on-cloud-architecture#hardware-specifications-on-cloud) for available machine types.
