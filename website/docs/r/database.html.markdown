...
- `key_protect_key` - (Optional, Forces new resource, String) The root key CRN of a Key Management Services like Key Protect or Hyper Protect Crypto Service (HPCS) that you want to use for disk encryption. A key CRN is in the format `crn:v1:<…>:key:`. You can specify the root key during the database creation only. After the database is created, you cannot update the root key. For more information, refer [Disk encryption](https://cloud.ibm.com/docs/cloud-databases?topic=cloud-databases-key-protect#using-the-key-protect-key) documentation.
  ...
  Using a customer-managed key requires a service-to-service authorization that grants the database service the `Reader` and `Authorization Delegator` roles on the key management service. Create it with an `ibm_iam_authorization_policy` resource. When the policy is created in the same configuration as the database, add an explicit `depends_on` from the database to the policy so it is created first.

  IAM needs a short time to propagate a new authorization policy before the database service can use it, and a policy narrowed with `source_resource_group_id` takes longer to become visible than an account-wide one. During that window the provisioning API rejects the request with `Missing or mis-configured S2S Authorization Policy`. The provider retries creation while this rejection persists, so a correct policy that is merely slow to propagate no longer fails the apply. If the authorization is genuinely missing or scoped to a different resource group than the one the instance is created in, creation still fails once the retry budget is exhausted, with guidance on the policy to create.
