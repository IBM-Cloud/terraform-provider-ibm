...
- `async_restore` - (Optional, Boolean) Should only be set when promoting an asynchronous restore. By setting this value to `true`, restore the first member synchronously, then start restoring the second member asynchronously. The default is `false`
