...
- `offering` - (Optional, String) The cluster offering type. Supported values are `kubernetes` (IBM Cloud Kubernetes Service cluster, default), `openshift` (Red Hat OpenShift on IBM Cloud cluster), and `openshift-vs` (Red Hat OpenShift Virtualization Service cluster).
