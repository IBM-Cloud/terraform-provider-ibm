...
- `flavor` - (Required, String) The flavor of the VPC worker node that you want to use. Only affecting the cluster creation and has no other effect later, to modify the workerpool use its dedicated resource. Only affecting the cluster creation and has no other effect later, to modify the workerpool use its dedicated resource.
...
- `host_pool_id` - (Optional, String) If provided, the cluster will be associated with a dedicated host pool identified by this ID. Only affecting the cluster creation and has no other effect later, to modify the workerpool use its dedicated resource.
...
- `secondary_storage` - (Optional, String) The secondary storage option for the default worker pool. Only affecting the cluster creation and has no other effect later, to modify the workerpool use its dedicated resource.
...
- `vpc_id` - (Required, String) The ID of the VPC that you want to use for your cluster. To list available VPCs, run `ibmcloud is vpcs`. Only affecting the cluster creation and has no other effect later, to modify the workerpool use its dedicated resource.
- `zones` - (Required, List) A nested block describes the zones of this VPC cluster's default worker pool. Only affecting the cluster creation and has no other effect later, to modify the workerpool use its dedicated resource.
