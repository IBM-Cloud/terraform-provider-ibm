...
* `build_number` - (Integer) The latest pipeline run build number. If this property is absent, the pipeline hasn't had any pipelineRuns.
  * Constraints: The minimum value is `1`.
* `created` - (String) Standard RFC 3339 Date Time String.
...
Nested scheme for **definitions**:
	...
	* `service_instance_id` - (String) UUID.
	  * Constraints: The maximum length is `36` characters. The minimum length is `36` characters. The value must match regular expression `/^[-0-9a-z]+$/`.
* `enabled` - (Boolean) Flag whether this pipeline enabled.
...
* `pipeline_definition` - (List) Tekton pipeline definition document detail object. If this property is absent, the pipeline has no definitions added.
Nested scheme for **pipeline_definition**:
	* `id` - (String) UUID.
	  * Constraints: The maximum length is `36` characters. The minimum length is `36` characters. The value must match regular expression `/^[-0-9a-z]+$/`.
	* `status` - (String) The state of pipeline definition status.
...
Nested scheme for **toolchain**:
	* `crn` - (String) The CRN for the toolchain that containing the tekton pipeline.
	  * Constraints: The maximum length is `512` characters. The minimum length is `9` characters. The value must match regular expression `/^crn:v[0-9](:([A-Za-z0-9-._~!$&'()*+,;=@\/]|%[0-9A-Z]{2})*){8}$/`.
	* `id` - (String) UUID.
	  * Constraints: The maximum length is `36` characters. The minimum length is `36` characters. The value must match regular expression `/^[-0-9a-z]+$/`.
* `triggers` - (List) Tekton pipeline triggers list.
Nested scheme for **triggers**:
	* `concurrency` - (List) Concurrency object.
	Nested scheme for **concurrency**:
		* `max_concurrent_runs` - (Integer) Defines the maximum number of concurrent runs for this trigger.
		  * Constraints: The minimum value is `0`.
	* `cron` - (String) Needed only for timer trigger type. Cron expression for timer trigger. Maximum frequency is every 5 minutes.
	...
	* `disabled` - (Boolean) flag whether the trigger is disabled.
	* `event_listener` - (String) Event listener name.
	  * Constraints: The maximum length is `253` characters. The minimum length is `1` character. The value must match regular expression `/^[-0-9a-zA-Z_.]{1,235}$/`.
	* `events` - (List) Needed only for git trigger type. Events object defines the events this git trigger listening to.
	Nested scheme for **events**:
		* `pull_request` - (Boolean) If true, the trigger starts when tekton pipeline service receive a repo pull reqeust's 'open' or 'update' git webhook event.
		* `pull_request_closed` - (Boolean) If true, the trigger starts when tekton pipeline service receive a repo pull reqeust's 'close' git webhook event.
		* `push` - (Boolean) If true, the trigger starts when tekton pipeline service receive a repo's 'push' git webhook event.
	* `id` - (String) Id.
	  * Constraints: The maximum length is `36` characters. The minimum length is `36` characters. The value must match regular expression `/^[-0-9a-z]+$/`.
	* `name` - (String) name of the duplicated trigger.
	  * Constraints: The maximum length is `253` characters. The minimum length is `1` character. The value must match regular expression `/^[a-zA-Z0-9][-0-9a-zA-Z_. ]{1,235}[a-zA-Z0-9]$/`.
	* `properties` - (List) Trigger properties.
	Nested scheme for **properties**:
		...
		* `value` - (String) String format property value.
		  * Constraints: The maximum length is `4096` characters. The minimum length is `1` character. The value must match regular expression `/./`.
	* `scm_source` - (List) Scm source for git type tekton pipeline trigger.
	Nested scheme for **scm_source**:
		* `blind_connection` - (Boolean) Needed only for git trigger type. Branch name of the repo.
		* `branch` - (String) Needed only for git trigger type. Branch name of the repo. Branch field doesn't coexist with pattern field.
		  * Constraints: The maximum length is `253` characters. The minimum length is `1` character. The value must match regular expression `/^[-0-9a-zA-Z_.]{1,235}$/`.
		* `hook_id` - (String) Webhook ID.
		  * Constraints: The maximum length is `253` characters. The minimum length is `1` character. The value must match regular expression `/^[-0-9a-zA-Z_.]{1,235}$/`.
		* `pattern` - (String) Needed only for git trigger type. Git branch or tag pattern to listen to. Please refer to https://github.com/micromatch/micromatch for pattern syntax.
		...
		* `url` - (Forces new resource, String) Needed only for git trigger type. Repo URL that listening to.
		  * Constraints: The maximum length is `2048` characters. The minimum length is `10` characters. The value must match regular expression `/^http(s)?:\/\/([^\/?#]*)([^?#]*)(\\?([^#]*))?(#(.*))?$/`.
	* `secret` - (List) Needed only for generic trigger type. Secret used to start generic trigger.
	Nested scheme for **secret**:
		* `algorithm` - (String) Algorithm used for "digestMatches" secret type.
	...
	* `service_instance_id` - (String) UUID.
	  * Constraints: The maximum length is `36` characters. The minimum length is `36` characters. The value must match regular expression `/^[-0-9a-z]+$/`.
	* `source_trigger_id` - (String) source trigger ID to clone from.
	  * Constraints: The maximum length is `36` characters. The minimum length is `36` characters. The value must match regular expression `/^[-0-9a-z]+$/`.
	* `tags` - (List) Trigger tags array.
	  * Constraints: The list items must match regular expression `/^[-0-9a-zA-Z_.]{1,235}$/`.
	* `timezone` - (String) Needed only for timer trigger type. Timezones for timer trigger.
	...
	* `worker` - (List) Trigger worker used to run the trigger, the trigger worker overrides the default pipeline worker.If not exist, this trigger uses default pipeline worker.
	Nested scheme for **worker**:
		* `id` - (String) Id.
