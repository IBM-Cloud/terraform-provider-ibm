...
description: |-
  Manages is_dynamic_route_server_peer_group_peer.
subcategory: "Virtual Private Cloud API"
---

# ibm_is_dynamic_route_server_peer_group_peer

Create, update, and delete is_dynamic_route_server_peer_group_peers with this resource.
...
* `name` - (Optional, String) The name for this dynamic route server peer group peer. The name must not be used by another peer in the peer group.
  * Constraints: The maximum length is `63` characters. The minimum length is `1` character. The value must match regular expression `/^-?([a-z]|[a-z][-a-z0-9]*[a-z0-9]|[0-9][-a-z0-9]*([a-z]|[-a-z][-a-z0-9]*[a-z0-9]))$/`.
...
* `href` - (String) The URL for this dynamic route server peer group peer.
  * Constraints: The maximum length is `8000` characters. The minimum length is `10` characters. The value must match regular expression `/^http(s)?:\/\/([^\/?#]*)([^?#]*)(\\?([^#]*))?(#(.*))?$/`.
* `is_dynamic_route_server_peer_group_peer_id` - (String) The ID for this dynamic route server peer group peer.
  * Constraints: The maximum length is `64` characters. The minimum length is `1` character. The value must match regular expression `/^[-0-9a-z_]+$/`.
...
* `etag` - ETag identifier for is_dynamic_route_server_peer_group_peer.

## Import

You can import the `ibm_is_dynamic_route_server_peer_group_peer` resource by using `id`.
The `id` property can be formed from `dynamic_route_server_id`, `dynamic_route_server_peer_group_id`, and `is_dynamic_route_server_peer_group_peer_id` in the following format:

<pre>
&lt;dynamic_route_server_id&gt;/&lt;dynamic_route_server_peer_group_id&gt;/&lt;is_dynamic_route_server_peer_group_peer_id&gt;
</pre>
...
* `is_dynamic_route_server_peer_group_peer_id`: A string in the format `r006-8228af60-189d-11ed-861d-0242ac120004`. The ID for this dynamic route server peer group peer.

# Syntax
<pre>
$ terraform import ibm_is_dynamic_route_server_peer_group_peer.is_dynamic_route_server_peer_group_peer &lt;dynamic_route_server_id&gt;/&lt;dynamic_route_server_peer_group_id&gt;/&lt;is_dynamic_route_server_peer_group_peer_id&gt;
</pre>
